...
class Trainer():
    ...
    def train(self):
        ...
        for epoch in range(self.epoches):
            ...
            if mark == 'save':
                ...
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_OVLAFND_weibo21.pkl')))
        print("开始进行最后的测试")
    ...
    def save_training_report(self):
        ...
        plt.plot(self.history['epoch'], self.history['logic_loss_neg'], label='Negative Logic', color='orange',
                 alpha=0.6)
        plt.plot(self.history['epoch'], self.history['loss_align'], label='align Logic', color='blue',
                 alpha=0.6)
