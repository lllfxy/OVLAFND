...
def create_weibo21_fixed_val_splits(train_file, val_file, test_file, output_base_dir):
    """
    针对 Weibo21 数据集 (.xlsx)
    策略：动态调整验证集分配。指定部分领域的验证集为"教育考试"，其余保持原有的"社会生活"。
    """
    ...
    for test_domain in domains:
        # === 【核心修改：验证集选择逻辑】 ===
        # 将你指定的 5 个领域测试集对应的验证集替换为 '教育考试'
        if test_domain in ['军事']:
            # val_domain = '教育考试'
            val_domain = '文体娱乐'
        elif test_domain in ['社会生活']:
            val_domain = '医药健康'
        elif test_domain in ['科技', '政治']:
            val_domain = '教育考试'
        elif test_domain in ['灾难事故', '文体娱乐']:
            # val_domain = '教育考试'
            val_domain = '军事'

        else:
            # 其他领域不修改，保留原代码默认逻辑，使用 '社会生活' 作为验证集
            val_domain = '社会生活'
        # ===================================
